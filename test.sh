...
# 10. multiple values with quoting preserve spaces
check_eq "quoted values" "a b,c d" "$(echo '%X,%Y' | "$BIN" %X 'a b' %Y 'c d')"

# 11. stats report unused variables on stderr
...
# 12. help exits successfully and mentions usage
help_output="$("$BIN" --help 2>&1)"
check_contains "help output" "Usage:" "$help_output"

# 13. invalid option exits non-zero
